...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *estateButtonHeightConstraint;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *estateLineHeightConstraint;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.bottomViewConstraint.constant = BottomMargin;
...
{
    if (self.otherBlock) { //如果有其他回调事件
        self.estateButtonHeightConstraint.constant = 57.0;
        self.estateLineHeightConstraint.constant = 0.5;
        [self.btnAblum setTitle:@"手机相册上传" forState:UIControlStateNormal];
    }else {
        self.estateButtonHeightConstraint.constant = 0.0;
        self.estateLineHeightConstraint.constant = 0.0;
        [self.btnAblum setTitle:@"相册上传" forState:UIControlStateNormal];
    }
...
- (void)previewSelectedPhotos:(NSArray<UIImage *> *)photos assets:(NSArray<PHAsset *> *)assets index:(NSInteger)index
{
    ...
    [svc setBtnDonePreviewBlock:^(NSArray<UIImage *> *photos, NSArray<PHAsset *> *assets) {
        ...
        [strongNav dismissViewControllerAnimated:YES completion:^{
            if (strongSelf.completBlock) {
                strongSelf.completBlock();
            }
        }];
    }];
    self.preview = NO;
...
- (IBAction)btnCamera_Click:(id)sender
{
    if (![self judgeIsHaveCameraAuthority]) {
        NSString *message = [NSString stringWithFormat:GetLocalLanguageTextValue(ZLPhotoBrowserNoCameraAuthorityText), @"易居房友经纪"];
...
- (IBAction)btnEstate_Click:(UIButton *)sender {
    if (self.otherBlock) {
        self.otherBlock();
    }
    [self hide];
...
- (void)requestSelPhotos:(UIViewController *)vc
{
    ...
    if (self.arrSelectedModels.count>0) {
        ...
        if (self.selectImageBlock) {
            self.selectImageBlock([[NSMutableArray alloc] init], [[NSMutableArray alloc] init], self.isSelectOriginalPhoto);
        }
        [self hide];
...
{
    ZLPhotoModel *model = self.arrDataSources[indexPath.row];
    CGSize size = [self getSizeWithAsset:model.asset];
    return size;
}
...
- (void)handleDataArray:(ZLPhotoModel *)model
{
    ...
    if (self.originalIndex == -1) {
        ...
        if (self.maxSelectCount > 1) {
            model.isSelected = YES;
            if (self.arrSelectedModels.count > self.originalIndex) {
                [self.arrSelectedModels replaceObjectAtIndex:self.originalIndex withObject:model];
            }else {
                [self.arrSelectedModels addObject:model];
            }
        }
