...
//    _imageView.frame = CGRectMake((kViewWidth-w)/2, (kViewHeight-h)/2-60, w, h);
    CGFloat w = kViewWidth;
    CGFloat h = w * self.model.asset.pixelHeight / self.model.asset.pixelWidth;
    if (h > kViewHeight-50) {
        h = kViewHeight-50;
        w = h * self.model.asset.pixelWidth / self.model.asset.pixelHeight;
    }
    _imageView.frame = CGRectMake((kViewWidth-w)/2, (kViewHeight-h-50)/2, w, h);
    _gridLayer.frame = _imageView.bounds;
    self.clippingRatio = [[ZLRatio alloc] initWithValue1:1 value2:1];
//    [self clippingRatioDidChange];
    _bottomView.frame = CGRectMake(0, kViewHeight-50, kViewWidth, 50);
    _bottomView.backgroundColor = color_black3;
...
- (void)initUI
{
    ...
    _gridLayer.bgColor   = [[UIColor blackColor] colorWithAlphaComponent:0.4];
...
//    _cancelBtn.frame = CGRectMake(10, 7, GetMatchValue(GetLocalLanguageTextValue(ZLPhotoBrowserCancelText), 15, YES, 30), 30);
    _cancelBtn.frame = CGRectMake(15, 0, 65, 50);
    ...
    _doneBtn.frame = CGRectMake(kViewWidth-80, 0, 65, 50);
...
- (void)loadImage
{
    ...
    [ZLPhotoManager requestImageForAsset:self.model.asset size:size completion:^(UIImage *image, NSDictionary *info) {
        if (![[info objectForKey:PHImageResultIsDegradedKey] boolValue] && image && image.size.width>0 && image.size.height>0) {
            strongify(weakSelf);
