...
- (void)viewDidLoad {
    ...
    self.selectCountLabel.layer.cornerRadius = 21.0f;
    self.selectCountLabel.clipsToBounds = YES;
    ...
    [self showRightButtonWithImage:[UIImage imageNamed:@"submit_red.png"] title:@"" addTarget:self action:@selector(btnDone_Click:)];
    
    if (nav.editAfterSelectThumbnailImage || nav.maxSelectCount==1) {
...
- (void)resetBottomBtnsStatus
{
    ...
    self.selectCountLabel.text = [NSString stringWithFormat:@"%ld",nav.arrSelectedModels.count];
    BOOL canEdit = NO;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (nav.editAfterSelectThumbnailImage &&
        nav.allowEditImage &&
        nav.maxSelectCount == 1) {  //需要编辑图片 且选择图片最大数量为1时
        [nav.arrSelectedModels addObject:model];
        [self btnEdit_Click:nil];
        return;
    }
    
    //不需要编辑图片 且选择图片最大数量为1时
    if (!nav.editAfterSelectThumbnailImage && nav.maxSelectCount==1) {
        [nav.arrSelectedModels addObject:model];
        ZLImageNavigationController *nav = (ZLImageNavigationController *)self.navigationController;
        if (nav.callSelectImageBlock) {
